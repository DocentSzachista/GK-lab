...
# color = vec4(0.7, 0.7, 0.7, 1.0); 3.0 podmiana color z tego na vertex_color w vertex_shader source i fragment
def compile_shaders():
    vertex_shader_source = """
        ...
        // https://www.shadertoy.com/view/4djSRW  function for generating deformations 
       float rand(vec2 co)
        {
            return fract(sin(dot(co.xy ,vec2(gl_InstanceID, gl_VertexID))) * 43758.5453);
        }
        ...
        void main(void) {
            ...
            gl_Position = P_matrix * V_matrix * M_matrix * (position + vec4(gl_InstanceID % 10+ rand(position.xy), gl_InstanceID/(10+rand(position.xy) ), 0, 0));
            vertex_color =  colors;
        }
    """
...
# ciag dalszy nastapi 
def draw_multiple_objects(M_location, M_matrix, dim_x, dim_y):
...
def render(time):
    ...
    #draw_multiple_objects(M_location, M_matrix, 10, 10)
